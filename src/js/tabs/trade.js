...
TradeTab.prototype.angular = function(module)
{
                                  ...
                                  function (books, $scope, $id, $network, $routeParams, $location, $filter, $rpTracker)
  {
    ...
    $scope.update_first = function (type) {
      var order = $scope.order[type];
      var first_currency = $scope.order.first_currency || "LAT";
      var formatted = "" + order.first + " " + first_currency;

      order.first_amount = ripple.Amount.from_human(formatted);

      if (first_currency !== 'LAT') order.first_amount.set_issuer($scope.order.first_issuer);
    };

    $scope.update_price = function (type) {
      var order = $scope.order[type];
      var second_currency = $scope.order.second_currency || "LAT";
      var formatted = "" + order.price + " " + second_currency;

      order.price_amount = ripple.Amount.from_human(formatted);

      if (second_currency !== 'LAT') order.price_amount.set_issuer($scope.order.second_issuer);
    };
    ...
    $scope.update_second = function (type) {
      ...
      if (second_currency !== 'LAT') order.second_amount.set_issuer($scope.order.second_issuer);
    };
    ...
    function updateSettings() {
      ...
      if ("string" !== typeof pair || !pair.match(/^[a-z]{3}\/[a-z]{3}$/i)) {
        order.first_currency = 'LAT';
        order.second_currency = 'LAT';
      ...
      // Invalid issuers or LAT/LAT pair
      if ((first_currency !== 'LAT' && !first_issuer.is_valid()) ||
          (second_currency !== 'LAT' && !second_issuer.is_valid()) ||
          (first_currency === 'LAT' && second_currency === 'LAT')) {
        order.valid_settings = false;
        return;
      }
      ...
      var key = "" +
        ...
        (order.first_currency === 'LAT' ? "" : "/" + order.first_issuer) +
        ":" +
        order.second_currency +
        (order.second_currency === 'LAT' ? "" : "/" + order.second_issuer);
    ...
    ['first','second'].forEach(function(prefix){
      ...
      $scope['save_' + prefix + '_issuer'] = function () {
        ...
        if ($scope.order.valid_settings && $scope.order[prefix + '_currency'] !== 'LAT') {
          if (prefix === 'first') {
    ...
    var updateCanBuySell = function () {
      ...
      var canBuy = second_currency.toUpperCase() === 'LAT' ||
          ($scope.balances[second_currency] && $scope.balances[second_currency].total.is_positive());
      var canSell = first_currency.toUpperCase() === 'LAT' ||
